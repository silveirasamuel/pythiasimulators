    ...
    for (int iEvent = 0; iEvent<nEvents;++iEvent){
        ...
        for(Int_t i=0;i<pythia.event.size();i++){
            if(pythia.event[i].id() == -24){
            	//if(pythia.event[pythia.event[i].mother2()].id() == 2 || pythia.event[pythia.event[i].mother1()].id()==2){
            		if(1)/*pythia.event[pythia.event[i].daughter1()].id() == 3 || pythia.event[pythia.event[i].daughter1()].id() == -4)*/{
            			if(1)/*pythia.event[pythia.event[i].daughter2()].id() == 3 || pythia.event[pythia.event[i].daughter2()].id() == -4)*/{
            		
				            MParticle *mypart = (MParticle*)(particleArray->New(ipart++));
				            mypart->id = i;
				            mypart->pdgcode = pythia.event[i].id();
                            cout<<"Meu pdg"<<pythia.event[i].id();
                            ...
                            cout<<"Eu_i:"<<i<<"Mãe1_i"<<pythia.event[i].mother1();
                            aux =  pythia.event[i].mother1();
                            cout<<" Mãe 1:"<<pythia.event[aux].id();
				            mypart->mother2 = (pythia.event[i].mother2());
                            aux =  pythia.event[i].mother2();
                            cout<<" Mãe2_i"<<pythia.event[i].mother2()<<" Mãe 2:"<<pythia.event[aux].id();
				            mypart->daughter1 = (pythia.event[i].daughter1());
                            aux = pythia.event[i].daughter1();
                            cout<<" Filha1_i"<<pythia.event[i].daughter1()<<" Filha 1:"<<pythia.event[aux].id();
				            mypart->daughter2 = (pythia.event[i].daughter2());
                            aux = pythia.event[i].daughter2();
                            cout<<" Filha2_i"<<pythia.event[i].daughter2()<<" Filha 2:"<<pythia.event[aux].id()<<endl;
            			}
            		}
            	//}
           	}
